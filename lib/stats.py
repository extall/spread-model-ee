...
def ee_read_covid19_stats(url=None):
    # Get the data from the internet
    if url is None:
        url = EE_URL_COVID19_TEST_RESULTS_PER_COUNTY
...
class InfectedPerson:
    ...
    def __init__(self, pers_id, when_test, area_id, logdata, logfn=None):
        ...
        txt = "Person " + str(self.pers_id) + " was diagnosed with COVID19 on " +\
              str(self.timestamp) + " in the area " + str(self.area_id)
    ...
    def check_recovered(self, new_date, logdata, logfn=None):
        ...
        if days_passed.days > self.days_to_recovery:
            ...
            txt = "Person " + str(self.pers_id) + " recovered from COVID19 on " + \
                  str(new_date) + " in the area " + str(self.area_id)
...
def ee_parse_infected_dynamically_and_assign_to_areas_until_date(mydate, log_redirect_fn=None,  url=None):

    logdata = [] # Collect interesting information about the infection dynamics

    # Parse the dataframe
    txt = "Parsing the data about COVID19 occurence in Estonia according " + \
          "to official information and simulating with a statistical model for recovery..."
    if log_redirect_fn is None:
        print(txt)
    else:
        log_redirect_fn(txt)

    # First, get the fresh stats
    data = ee_read_covid19_stats(url=url)
    ...
    for row in data.iterrows():
        ...
        [person.check_recovered(current_day, logdata, logfn=log_redirect_fn) for person in the_infected if person.state == "I"]
        ...
        if county is not -1 and row[1]["ResultValue"] == "P":
            ...
            person = InfectedPerson(the_inf_id, current_day, county_to_area[county], logdata, logfn=log_redirect_fn)
            the_infected.append(person)
            the_inf_id += 1

    txt = "Finished parsing."
    if log_redirect_fn is None:
        print(txt)
    else:
        log_redirect_fn(txt)
